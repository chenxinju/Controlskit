 ...
 */
+ (void)showCustomTipHUD:(NSString *)message iscustomView:(BOOL)iscustomView position:(MBProgressHUBPosition)position isLineFeed:(BOOL)isLineFeed   backgroundColor:(UIColor *_Nullable )backgroundColor textColor:(UIColor *_Nullable)textColor textFont:(UIFont *_Nullable)textFont margin:(CGFloat)margin offset:(CGPoint)offset isWindow:(BOOL)isWindow timer:(NSTimeInterval)time confirmBlock:(void (^)(void ))confirmBlock;


/// 展示自定义view带事件提示语
/// @param message 提示语
/// @param isWindow 是否展示在Window上
 ...
 */
+ (void)showGifImageHUDInView:(UIImage * _Nullable)gifImage message:(NSString *)message backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor textFont:(UIFont *)textFont alpha:(CGFloat)alpha timer:(NSTimeInterval)aTimer isCancel:(BOOL)cancel closeBlock:(void(^_Nonnull)(void))closeBlock;
...
@property(nonatomic,strong) CDTouploadTipView * customTipView;
@end
