...
//  DIYPlusObjc
//
//  Created by iOS1 on 2022/3/22.
//  Copyright © 2020 CXJ-Build. All rights reserved.
//
...
+ (void)showCustomTipHUD:(NSString *)message iscustomView:(BOOL)iscustomView position:(MBProgressHUBPosition)position isLineFeed:(BOOL)isLineFeed backgroundColor:(UIColor *__nullable)backgroundColor textColor:(UIColor *__nullable )textColor textFont:(UIFont *__nullable)textFont margin:(CGFloat)margin offset:(CGPoint)offset isWindow:(BOOL)isWindow timer:(NSTimeInterval)time confirmBlock:(void(^)(void))confirmBlock{
    [self p_showCustomTipMessage:message iscustomView:iscustomView position:position isLineFeed:isLineFeed isWindow:isWindow backgroundColor:backgroundColor textColor:textColor textFont:textFont margin:margin offset:offset timer:time confirmBlock:confirmBlock];
}
...
/// 自定义文本框
+ (void)p_showCustomTipMessage:(NSString*)message iscustomView:(BOOL)iscustomView position:(MBProgressHUBPosition)position isLineFeed:(BOOL)isLineFeed isWindow:(BOOL)isWindow backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor textFont:(UIFont *)textFont margin:(CGFloat)margin offset:(CGPoint)offset timer:(NSTimeInterval)aTimer confirmBlock:(void(^)(void))confirmBlock{
    dispatch_main_sync_safe(^{
        MBProgressHUD *hud = [self p_createMBProgressHUDviewWithMessage:message isWindiw:isWindow];
        ...
        if (iscustomView) {
            ...
            CDTouploadTipView *customTipView = [[CDTouploadTipView alloc] initWithFrame:CGRectZero];
            customTipView.confirmBtnActionBlock = ^{
                confirmBlock();
            };
            customTipView.titleStr = message;
            hud.customView = customTipView;
            ...
            hud.bezelView.color = backgroundColor ?: [[UIColor blackColor] colorWithAlphaComponent:0.65];
            hud.mode = MBProgressHUDModeText;
            hud.bezelView.style = MBProgressHUDBackgroundStyleSolidColor;
        ...
        if (!CGPointEqualToPoint(offset, CGPointZero)) {
           ...
           //需根据 hud.offset = offset;
            
            UIView *view = isWindow ? [self p_getKeyWindow] : [self p_getCurrentViewInVC];
            
            CGFloat margin = 92 + Height_TapBarSafeBottomMargin;  //距离底部和顶部的距离
...
+ (void)showAlertMessage:(NSString *)message isWindow:(BOOL)isWindow position:(MBProgressHUBPosition)position timer:(NSTimeInterval)aTimer confirmBlock:(void(^)(void))confirmBlock{
    ...
    dispatch_main_sync_safe(^{
        ...
        NSLog(@"view--%@ offSetY--%.2f",view,offSetY);
...
+ (void)p_showGifImageHUD:(UIImage *)gifImage message:(NSString *)message isWindow:(BOOL)isWindow timer:(NSTimeInterval)aTimer backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor textFont:(UIFont *)textFont alpha:(CGFloat)alpha {
    dispatch_main_sync_safe(^{
        ...
        hud.minSize = CGSizeMake(187, 140);
...
/**
 在view上展示自定义GIF图片 - 不自动移除 可控制底部取消按钮
 ...
 @param alpha 透明度
 @param aTimer 移除时长
 @param cancel 是否底部显示取消按钮
 @param closeBlock 取消回调
 */
+ (void)showGifImageHUDInView:(UIImage *)gifImage message:(NSString *)message backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor textFont:(UIFont *)textFont alpha:(CGFloat)alpha timer:(NSTimeInterval)aTimer isCancel:(BOOL)cancel closeBlock:(void(^_Nonnull)(void))closeBlock {
    
    //强制主线程执行
    dispatch_main_sync_safe(^{
        MBProgressHUD *hud  =  [self p_createMBProgressHUDviewWithMessage:message isWindiw:NO];
        hud.mode = MBProgressHUDModeCustomView;
        hud.bezelView.layer.cornerRadius = 9.0;//设置背景框圆角
        UIImage *lodGiftImage = gifImage;
        if (lodGiftImage == nil) {
            lodGiftImage = [XWHUDManager imageGIFWithData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"update_loading" ofType:@"gif"]]];
        }
        UIImageView *gifImageView = [[UIImageView alloc] initWithImage:lodGiftImage];
        hud.customView = gifImageView;
        ...
        hud.minSize = CGSizeMake(187, 140);
//        hud.detailsLabel.text = @"若图片较大/网速较慢，请耐心等待";
        ...
        if (cancel) {
            ...
            [hud.button setTitleColor:[UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1.0] forState:UIControlStateNormal];
            
            //addTarget 可使用 progressObject
            // [hud.button addTarget:self action:@selector(cancelaction) forControlEvents:UIControlEventTouchUpInside];
            [[hud.button rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        [hud hideAnimated:YES afterDelay:(aTimer > 0 ? aTimer : HUGE_VALF)];
    });
...
+ (MBProgressHUD *)p_createMBProgressHUDviewWithMessage:(NSString*)message isWindiw:(BOOL)isWindow {
    ...
    hud.bezelView.style = MBProgressHUDBackgroundStyleSolidColor; //黑色：MBProgressHUDBackgroundStyleSolidColor  白色：MBProgressHUDBackgroundStyleBlur
    // 注释下面配置代码默认显示浅灰->
